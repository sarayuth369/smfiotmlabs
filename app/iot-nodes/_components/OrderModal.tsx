...
import { HARDWARE, type SKU } from "@/lib/hardware";
import { createHardwareOrder, pollHardwareOrder } from "../actions";
...
export function OrderModal({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const res = await createHardwareOrder(sku);
      if (cancelled) return;
      if (!res.ok) {
        setState("error");
        setError(res.error);
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(async () => {
      const res = await pollHardwareOrder(pid);
    ...
    >
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-start justify-between">
          <div>
            <div className="text-xs text-brand-700/70 font-medium">สั่งซื้ออุปกรณ์</div>
            <div className="text-lg font-bold text-brand-800">{item.name}</div>
          </div>
          <button
            onClick={onClose}
          ...
          {state === "succeeded" ? (
            <div className="text-center py-4">
              ...
              <p className="mt-2 text-sm text-brand-900/70">
                คำสั่งซื้อ <span className="font-semibold">{item.name}</span> บันทึกเรียบร้อยแล้ว
                <br />ทีมงานจะติดต่อคุณเพื่อยืนยันการจัดส่งเร็ว ๆ นี้
              </p>
              <a
                href="/dashboard"
