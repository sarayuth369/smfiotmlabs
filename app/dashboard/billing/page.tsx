...
import { getUserPlan, formatPlanLabel, formatLimit } from "@/lib/plan-limits";
import { RenewStarterButton } from "./_components/RenewStarterButton";
      ...
      <div className="card p-6 mt-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-2 text-xs text-brand-900/55">
              ขีดจำกัด: {formatLimit(plan.limits.max_farms)} ฟาร์ม ·{" "}
              {formatLimit(plan.limits.max_zones)} โซน · {formatLimit(plan.limits.max_nodes)} อุปกรณ์ ·{" "}
              {formatLimit(plan.limits.max_sensors)} เซนเซอร์
            </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2">
          {sub.set_plan === "starter" && sub.status === "expired" ? (
            <RenewStarterButton />
          ) : (
            <Link
              href="/pricing"
