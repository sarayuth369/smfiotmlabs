...
export async function logAiRequest(
  ...
  await admin.from("ai_requests").insert({
    ...
    zone_signature: row.zone_signature ?? null,
    ok: row.ok,
 ...
 * analysis for the same period AND the same zone/crop context — must match
 * zone_signature too, otherwise editing a Zone's crop and re-analyzing within
 * the cache window would silently return advisory text for the old crop.
 */
export async function findCachedAnalysis(
  ...
  const { data } = await admin
    ...
    .eq("zone_signature", zoneSignature)
    .eq("ok", true)
    .not("result", "is", null)
